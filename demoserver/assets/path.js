$(function() {
	var container = $("body .path-panel"),
		preList = container.find("ul").first(),
		nextList = container.find("ul").last(),
		centerTarget = container.find(".path-center"),
		fromPath = container.find("canvas").first(),
		toPath = container.find("canvas").last(),
		hDelta = 52 + 5 + 3,
		ctxF = fromPath[0].getContext('2d'),
		ctxT = toPath[0].getContext('2d');
	ctxF.beginPath();


	for (var i = 0; i < 10; i++) {
		ctxF.moveTo(0, 20 + i * hDelta);
		ctxF.quadraticCurveTo(100, 270, 227, 270);
		ctxF.lineWidth = 3.0;
		// ctxF.lineJoin = 'round';
		ctxF.strokeStyle = "rgb(79,190,232)";
		ctxF.stroke();
	};
	...
	for (var i = 0; i < 10; i++) {
		ctxT.moveTo(0, 270);
		ctxT.quadraticCurveTo(100, 270, 230, 20 + i * hDelta);
		ctxT.lineWidth = 3.0;
		// ctxT.lineJoin = 'round';
		ctxT.strokeStyle = "rgb(255,204,21)";
		ctxT.stroke();
	};
